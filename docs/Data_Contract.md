...
## Week 12 Update — Airflow: переход на инкрементальный ETL и Quality Gate

### Orchestration Layer

На неделе 12 цепочка задач в Apache Airflow была полностью перестроена для обеспечения безопасности целевой базы данных. Теперь проверка качества данных (DQ) выполняется строго до шага загрузки. При обнаружении критических ошибок (статус `FAIL`) пайплайн останавливается, блокируя запись некорректных данных в хранилище.
...
| `transform` | Изолированное выполнение `.ipynb` ноутбука и сборка дневного CSV-марта за дату периода |
| `dq` | Шлюз качества: валидация сформированного марта текущего периода до фиксации в БД |
| `load` | Атомарная очистка старого инкремента и вставка новых данных в таблицу `mart_weather` |

---

### Schedule & Delivery Mode

Все слои пайплайна переведены на строго детерминированный инкрементальный режим (Daily partitioned). Работа с системным временем (`datetime.now()`) полностью устранена — каждый процесс обрабатывает изолированные файлы конкретного дня (`raw_YYYY-MM-DD.json`, `mart_daily_YYYY-MM-DD.csv`).
...
В скрипте `src/load.py` метод полной перезаписи всей таблицы `if_exists="replace"` заменен на транзакционный паттерн **Delete period + Insert** внутри единого контекста `engine.begin()`. 

При повторных запусках, перезапусках тасок или выполнении backfill за определенный день старые строки этого периода удаляются автоматически, полностью исключая дублирование бизнес-ключей (`date + city_id`).
